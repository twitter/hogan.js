...
(function (Hogan) {
  ...
  Hogan.stringify = function(codeObj, text, options) {
    var partials = [];
    var result = "{code: function (c,p,i) { " + Hogan.wrapMain(codeObj.code) + " }, partials: {";
    for (var key in codeObj.partials) {
      partials.push('"' + esc(key) + '": {name:"' + esc(codeObj.partials[key].name) + '"}');
    }
  ...
  Hogan.makePartials = function(codeObj) {
    var template = {subs: {}, partials: codeObj.partials, name: codeObj.name  };
    for (var key in template.partials) {
      template.partials[key] = this.makePartials(template.partials[key]);
    }
    addFunctionsToObject(codeObj.subs, template.subs);
    return template;
  }

  function addFunctionsToObject(source, target) {
